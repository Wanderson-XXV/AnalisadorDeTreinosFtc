...
export function CycleModal({ isOpen, onClose, onSubmit, cycleNumber, cycleDuration }: CycleModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      setHits(0);
      setMisses(0);
      // Focus no input de acertos quando o modal abre
      setTimeout(() => {
  ...
  const formatDuration = (ms: number) => {
    const safeMs = ms ?? 0;
    const secs = (safeMs / 1000).toFixed(2);
    return `${secs}s`;
  };
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100]">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="text-xl font-bold text-white">Ciclo #{cycleNumber ?? 0}</h3>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => setHits(Math.max(0, (hits ?? 0) - 1))}
              ...
              <input
                ...
                className="text-4xl font-bold text-green-400 w-24 text-center bg-slate-700/50 border border-slate-600 rounded-xl py-2 focus:outline-none focus:ring-2 focus:ring-green-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              <button
                onClick={() => setHits((hits ?? 0) + 1)}
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => setMisses(Math.max(0, (misses ?? 0) - 1))}
                className="p-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-white transition-colors"
                tabIndex={-1}
              >
                <Minus className="w-6 h-6" />
              ...
              <input
                ...
                onKeyDown={handleKeyDown}
                className="text-4xl font-bold text-red-400 w-24 text-center bg-slate-700/50 border border-slate-600 rounded-xl py-2 focus:outline-none focus:ring-2 focus:ring-red-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              <button
                onClick={() => setMisses((misses ?? 0) + 1)}
